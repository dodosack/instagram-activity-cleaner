...
(async function bulkUnlike() {
  ...
  // A 500 on the real unlike action always stops immediately - at that point
  ...
  while (!window.__STOP__ && total < MAX_TOTAL) {
    ...
    if (actionError && actionError !== 429) {
      ...
      console.warn(`Instagram returned HTTP ${actionError} on the unlike action - hard stop. Reload and wait 30-60+ min (sometimes hours) before trying again.`);
      break;
    }
    if (actionError === 429) {
      errorStreak++;
      ...
      console.warn(`HTTP 429 registered on the unlike action (retry ${errorStreak}/${MAX_RETRIES}) - backing off ~${Math.round(back / 1000)}s, then retrying still-liked posts. Warning: continuing after a 429 can result in a block.`);
      actionError = null;
