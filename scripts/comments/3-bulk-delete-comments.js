...
(async function bulkDeleteComments() {
  ...
  while (!window.__STOP__ && total < MAX_TOTAL) {
    ...
    if (httpError) {
      ...
      console.warn(`Instagram returned HTTP ${httpError} (rate limit). Backing off ~${Math.round(back / 1000)}s, then retrying (${errorStreak}/${MAX_ERRORS}). Remaining comments get retried.`);
      await sleep(back);
      continue;
    }
    errorStreak = 0;
